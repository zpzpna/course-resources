...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    ps_filter = [para for para in paragraphs if select(para)]
    if k <= len(ps_filter)-1:
        result = ps_filter[k]
    else:
        result = ''
    return result
...
def about(subject):
    ...
    def modify_word(word):
        """delete the character in word which is not in alphabet"""
        count = 0
        #直接遍历找要删除的删除后连接，本题要求是删掉字符串中所有非字母字符后匹配
        for i in word:
            if not (i>='a' and i<='z'):
                word = word[:count]+word[count+1:]
                continue #为什么用continue，因为对word遍历时，删去非法字符，此时count指向下一个新字符，不需要+1
            count = count+1
        return word
            
    def select(paragraph):
        paragraph_tmp = paragraph.lower().split()
        paragraph_tmp = [modify_word(word) for word in paragraph_tmp]
        for word in subject:
            if word in paragraph_tmp:
                return True
        return False
    return select
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    true_len = len([word_t for word_t,word_s in zip(typed_words,source_words) if word_t == word_s])
...
def wpm(typed, elapsed):
    ...
    last_time = elapsed/60
    return word_num/last_time
    # END PROBLEM 4

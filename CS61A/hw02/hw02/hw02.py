...
def funception(func1, start):
    ...
    def func2(stop):
            if start < 0:
                ...
                s = [func1(elem) for elem in range(start,stop)]
...
#important 
#if plateau,maintain the prev value ,if not, prev = curr 
def waves(n):
...
#!important tree recursion about choose mutiple ways  subtracting variable m
#(n-m,m)+(n,next(m))
#The situation doesn't allow duplicate results ,which means that if all the values are same,it's the duplicate results
#(like set ADT)
#however,func count_k allows duplicate results in different order(3-> 1+2/2+1/3/1+1+1)
